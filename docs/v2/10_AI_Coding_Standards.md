...
4.  **English Comments for Code, Chinese for Logic**
    *   **JSDoc / 函数头**: 英文 (English)。为了 Copilot 生成更准确，也为了看起来更标准。
    *   **复杂逻辑解释**: 中文 (Chinese)。为了解释 *为什么* 这么写，防止后续维护误解。

5.  **Test What You Break (Vitest)**
    *   所有 `src/v2/core` (纯逻辑层) 的代码必须有对应的单元测试。
    *   框架使用 `Vitest`。
